...
function añadirAlCarritoClickado(event){
    ...
    console.log(titulo)
    var precio = parseFloat(articuloTienda.getElementsByClassName('precio-producto')[0].innerText.replace('€',''))
    var imagenSrc = articuloTienda.getElementsByClassName('imagen-articulo')[0].src
    añadirItemAlCarrito(titulo, precio, imagenSrc)
    actualizarPrecioTotalCarrito()
}

function añadirItemAlCarrito(titulo, precio, imagenSrc){
    var carritoItem = document.createElement('div')
    carritoItem.classList.add('producto')
    ...
    var carritoItemContenido = `
        <div class="articulossss">
            <img src="${imagenSrc}" width="40" height="40" class="imagen-articulo">
            <span class="nombre-producto">${titulo}</span>
        <span class="precio-producto ariculossss">${precio}€</span>
...
function actualizarPrecioTotalCarrito(){
    ...
    var productos = contenedorObjetosCarrito.getElementsByClassName('producto')
    var total = 0;
